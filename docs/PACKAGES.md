...
| `internal/service/pilotprofile` | Pilot profile (adaptive disciplines): `derive.go` holds the pure `Derive` function (licences, ratings, licence privileges, fleet, flight aggregate, stored intents and `now` → one `DisciplineState` per discipline) and `PendingAcknowledgement`; `service.go` holds `Get`/`Update`/`Settings`/`Replace` over `PilotProfileRepository` and `DisciplineEvidenceSource`, with the privileges wired by `SetPrivilegeSource`. See [DOMAIN.md](./DOMAIN.md#pilot-profile-and-disciplines). |
| `internal/service/training` | Training progress (`GET /training/progress`): `templates.go` holds the syllabus templates and the pure `Evaluate` (programme + `repository.TrainingFlight` rows + SFCL.130(b) credit → `TrainingProgramme`); `service.go` picks the programmes from the pilot profile (`ProgrammesInTraining`) and the request, over `repository.TrainingRepository` (`postgres/training.go`). See [SAILPLANES.md](./SAILPLANES.md#training-progress). |
...
| `pkg/igc` | FAI IGC flight recorder files: `Parse` (A, H, I, B and E records; bounded input, typed `*ParseError`) and `Analyze` (take-off, landing, launch method with confidence, release height, maximum altitude, free and out-and-return distance). Pure, no I/O. Synthetic fixtures and their generator live in `testdata/`; `FuzzParse` fuzzes the parser. See [SAILPLANES.md](./SAILPLANES.md#igc-import). |
