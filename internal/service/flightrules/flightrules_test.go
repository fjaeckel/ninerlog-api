...
func TestDisplayPICName(t *testing.T) {
	...
	cases := []struct {
		...
		// Exam flights: the examiner is PIC of record.
...
// TestLogsPICOrDual asserts the PDF-export predicate keeps PIC, dual
// received and dual given rows and drops co-pilot-only ones.
func TestLogsPICOrDual(t *testing.T) {
	tests := []struct {
		name   string
		...
		{"nil", nil, false},
		{"pic flag", &models.Flight{IsPIC: true}, true},
		{"dual flag", &models.Flight{IsDual: true}, true},
		{"pic time only", &models.Flight{PICTime: 60}, true},
		{"dual time only", &models.Flight{DualTime: 60}, true},
		{"dual given time only", &models.Flight{DualGivenTime: 60}, true},
		{"co-pilot only", &models.Flight{TotalTime: 60, SICTime: 60}, false},
		{"nothing logged", &models.Flight{TotalTime: 60}, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := LogsPICOrDual(tt.flight); got != tt.want {
				t.Errorf("LogsPICOrDual = %v, want %v", got, tt.want)
			}
		})
	}
}

// TestFilterPICOrDual asserts the filter preserves order and never returns
// nil.
func TestFilterPICOrDual(t *testing.T) {
	pic := &models.Flight{AircraftReg: "D-EABC", IsPIC: true}
	sic := &models.Flight{AircraftReg: "D-ESIC", SICTime: 60}
	dual := &models.Flight{AircraftReg: "D-EDUA", IsDual: true}

	got := FilterPICOrDual([]*models.Flight{pic, sic, dual})
	if len(got) != 2 || got[0] != pic || got[1] != dual {
		t.Fatalf("FilterPICOrDual returned %d flights, want [D-EABC D-EDUA]", len(got))
	}
	if FilterPICOrDual(nil) == nil {
		t.Error("FilterPICOrDual(nil) must return an empty slice, not nil")
	}
}
