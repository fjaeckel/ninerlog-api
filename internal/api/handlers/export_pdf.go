...
func (h *APIHandler) ExportFlightsPDF(c *gin.Context) {
	...
	pdf.CellFormat(0, 4, fmt.Sprintf("Generated by PilotLog on %s — https://github.com/fjaeckel/pilotlog", time.Now().Format("02 Jan 2006 15:04 UTC")), "", 1, "C", false, 0, "")

	// Send PDF
	c.Header("Content-Type", "application/pdf")
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename=pilotlog_%s.pdf", time.Now().Format("2006-01-02")))
	pdf.Output(c.Writer)
}
