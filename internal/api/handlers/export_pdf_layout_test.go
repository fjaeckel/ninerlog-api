	...
	for _, size := range []string{"a4", "a5", "letter"} {
		...
		cases := []struct {
			...
			{"easa spread", batches*2 + 3, func() int {
				return renderEASA(flights, g, nil, "Test Pilot", layoutSpread).PageCount()
			}},
			{"easa single", batches + 1, func() int {
				return renderEASA(flights, g, nil, "Test Pilot", layoutSingle).PageCount()
			}},
			{"faa spread", batches*2 + 3, func() int {
				return generateFAAPDF(flights, g, "Test Pilot", layoutSpread).PageCount()
			}},
			{"faa single", batches + 1, func() int {
				return generateFAAPDF(flights, g, "Test Pilot", layoutSingle).PageCount()
			}},
			{"summary", 1, func() int {
				return generateSummaryPDF(flights, g, "Test Pilot").PageCount()
			}},
		}
...
func TestPDFLayoutEmptyLogbook(t *testing.T) {
	...
	for name, doc := range map[string]interface{ PageCount() int }{
		"easa spread": renderEASA(nil, g, nil, "", layoutSpread),
		"easa single": renderEASA(nil, g, nil, "", layoutSingle),
		"faa spread":  generateFAAPDF(nil, g, "", layoutSpread),
		"faa single":  generateFAAPDF(nil, g, "", layoutSingle),
		"summary":     generateSummaryPDF(nil, g, ""),
	} {
...
func TestPDFOutputIsValid(t *testing.T) {
	...
	for name, render := range map[string]func() *bytes.Buffer{
		"easa spread": func() *bytes.Buffer {
			var b bytes.Buffer
			if err := renderEASA(flights, g, nil, "Test Pilot", layoutSpread).Output(&b); err != nil {
				t.Fatal(err)
			}
			return &b
		},
		"faa single": func() *bytes.Buffer {
			var b bytes.Buffer
			if err := generateFAAPDF(flights, g, "Test Pilot", layoutSingle).Output(&b); err != nil {
				t.Fatal(err)
			}
			return &b
		},
