...
var customMetricSQL = map[string]string{
	...
	"picus_time":         "COALESCE(SUM(f.picus_time), 0)",
	"spic_time":          "COALESCE(SUM(f.spic_time), 0)",
	"examiner_time":      "COALESCE(SUM(f.examiner_time), 0)",
	"relief_time":        "COALESCE(SUM(f.relief_time), 0)",
...
var customMetricRowSQL = map[string]string{
	...
	"pic_time":           "f.pic_time",
	"picus_time":         "f.picus_time",
	"spic_time":          "f.spic_time",
	"examiner_time":      "f.examiner_time",
	"relief_time":        "f.relief_time",
...
func buildCustomWhere(userID uuid.UUID, since time.Time, filters []models.CurrencyFilter) (string, []interface{}, error) {
	...
	where := []string{"f.user_id = $1", "NOT f.is_simulator AND NOT f.is_passenger", "f.date >= $2"}
