...
func (r *licenseRepository) Create(ctx context.Context, license *models.License) error {
	query := `
		INSERT INTO licenses (
			...
			expiry_date, issuing_authority, is_active
...
func (r *licenseRepository) GetByID(ctx context.Context, id uuid.UUID) (*models.License, error) {
	query := `
		SELECT id, user_id, license_type, license_number, issue_date,
...
func (r *licenseRepository) GetActiveByUserID(ctx context.Context, userID uuid.UUID) ([]*models.License, error) {
	query := `
		SELECT id, user_id, license_type, license_number, issue_date,
		       expiry_date, issuing_authority, is_active, created_at, updated_at
		FROM licenses
...
func (r *licenseRepository) Update(ctx context.Context, license *models.License) error {
	query := `
		UPDATE licenses
		SET expiry_date = $1, is_active = $2, updated_at = $3
		WHERE id = $4
	`
