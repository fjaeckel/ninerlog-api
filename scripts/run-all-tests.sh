...
echo "�Checking that flight-rule logic is not duplicated outside flightrules/…"
...
docker compose -f docker-compose.test.yaml exec -T postgres-test psql -U testuser -d ninerlog_test < db/migrations/test_init.sql || echo "Migrations might need manual setup"
